...
class Action extends Table 
{
    ...
    {
        $param = ['row_name'=>'Action','col_label'=>'name'];
        parent::setup($param); 

        $start_default = date('Y-m-d');
        $end_default = date('Y-m-d',mktime(0,0,0,date('m')+12,date('j'),date('Y')));

        $this->addTableCol(array('id'=>'action_id','type'=>'INTEGER','title'=>'Action ID','key'=>true,'key_auto'=>true,'list'=>true));
        ...
        $this->addTableCol(array('id'=>'name','type'=>'STRING','title'=>'Action name'));
        ...
        $this->addTableCol(array('id'=>'progress_pct','type'=>'INTEGER','title'=>'Progress %','new'=>'0'));
        ...
        $this->addAction(array('type'=>'popup','text'=>'Owners','url'=>'action_owner','mode'=>'view','width'=>600,'height'=>600));
        ...
        $this->addSearch(array('action_id','link_to','name','description','repeat_freq',
                               'date_start','date_end','progress_pct','status'),array('rows'=>2));
